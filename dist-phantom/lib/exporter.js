...
const downloadCSV = (data, filename) => {
    const csvContent = [
        ...
        ...data.map(row => Object.keys(row).map(k => {
...
async function exportDashboardData(opts) {
    ...
    let data = [];
    let filename = '';
    const effectiveSheetId = sheetId || await (0, index_1.getSpreadsheet)() || undefined;
    ...
    if (type === 'holdings' || type === 'both') {
        ...
        if (!resolvedHoldings || resolvedHoldings.length === 0) {
            if (effectiveSheetId) {
                try {
                    const ports = await (0, index_1.fetchPortfolios)(effectiveSheetId);
                    resolvedHoldings = [];
                    ports.forEach(p => {
    ...
    let transactionsHeaders = [];
    if (type === 'transactions' || type === 'both') {
        if (!effectiveSheetId) {
            transactionsData = [];
        }
        else {
            const txns = await (0, index_1.fetchTransactions)(effectiveSheetId);
            let resolvedPortMap = portMap;
            if (!resolvedPortMap) {
                try {
                    const ports = await (0, index_1.fetchPortfolios)(effectiveSheetId);
                    resolvedPortMap = new Map(ports.map((p) => [p.id, p]));
                }
                catch (e) {
    ...
    if (format === 'csv') {
        if (type === 'holdings') {
            ...
            if ((holdingsData || []).length > 0)
                (0, exports.downloadCSV)(holdingsData, `holdings_${stamp}.csv`);
            if ((transactionsData || []).length > 0)
                (0, exports.downloadCSV)(transactionsData, `transactions_${stamp}.csv`);
        ...
        (0, exports.downloadCSV)(data, filename);
    ...
    try {
        ...
        const newSpreadsheetId = await (0, index_1.createEmptySpreadsheet)(title);
        if (!newSpreadsheetId)
            throw new Error('Failed to create spreadsheet');
        for (const sheet of sheetsToWrite) {
            const dataForSheet = sheet.data.map(row => sheet.headers.map(h => row[h]));
            await (0, index_1.exportToSheet)(newSpreadsheetId, sheet.sheetName, sheet.headers, dataForSheet);
        }
        // Delete the default "Sheet1"
        try {
            const sheetsResponse = await window.gapi.client.sheets.spreadsheets.get({ spreadsheetId: newSpreadsheetId });
            const sheet1 = sheetsResponse.result.sheets?.find((s) => s.properties.title === 'Sheet1');
