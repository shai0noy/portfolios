...
exports.synchronizeSeries = synchronizeSeries;
exports.synchronizeThreeSeries = synchronizeThreeSeries;
exports.computeSlope = computeSlope;
exports.computeAnalysisMetrics = computeAnalysisMetrics;
exports.calculateReturns = calculateReturns;
exports.normalizeToStart = normalizeToStart;
exports.getAnnualizationFactor = getAnnualizationFactor;
/**
 ...
 */
function synchronizeSeries(datasetX, datasetY) {
    if (datasetX.length === 0 || datasetY.length === 0)
        return [];
    const mapY = new Map();
 ...
 */
function synchronizeThreeSeries(datasetX, datasetY, datasetZ) {
    if (datasetX.length === 0 || datasetY.length === 0 || datasetZ.length === 0)
        return [];
    const mapY = new Map();
    const mapZ = new Map();
    const getDateKey = (ts) => {
 ...
 */
function computeAnalysisMetrics(returnPairs, riskFreeReturns, annualizationFactor = 252) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (x < 0)
            downsidePairs.push(returnPairs[i]);
    }
 ...
 */
function calculateReturns(pairs) {
    const returns = [];
 ...
 */
function normalizeToStart(data) {
    if (data.length === 0)
        return [];
    const first = data[0];
 ...
 */
function getAnnualizationFactor(dates) {
    if (dates.length < 2)
        return 252;
