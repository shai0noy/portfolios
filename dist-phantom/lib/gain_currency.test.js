...
(0, vitest_1.describe)('Gain Calculation with Currency Fluctuation', () => {
    ...
    (0, vitest_1.it)('should account for currency fluctuation (FX Gain) when initial rates are provided', () => {
        // Portfolio in ILS
        ...
        const lot = {
            ...
            costPerUnit: { amount: 50, currency: types_1.Currency.USD, rateToPortfolio: 3.0 },
            costTotal: { amount: 50, currency: types_1.Currency.USD, rateToPortfolio: 3.0 },
            feesBuy: { amount: 0, currency: types_1.Currency.USD, rateToPortfolio: 1 },
        ...
        (0, vitest_1.expect)(resBuggy.gain.valILS).toBe(0);
        ...
        const resFixed = h.generateGainForPeriod(new Date('2026-02-04'), provider, currentRates, historicalRates // This arg is ignored currently
        );
        // Currently expect 0 (since arg is ignored) - demonstrating the bug if we WANT FX gain.
        // After fix, this should be 50.
        (0, vitest_1.expect)(resFixed.gain.valILS).toBeCloseTo(50, 4);
    });
});
