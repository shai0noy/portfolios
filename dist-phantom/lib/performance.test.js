...
const mockFetchHistory = async (ticker, exchange, _signal) => {
...
async function testBasicBuyAndHold() {
    ...
    const txns = [{
            date: '2024-01-02', portfolioId: 'p1', ticker: 'AAPL', exchange: types_1.Exchange.NASDAQ,
            type: 'BUY', qty: 10, price: 102, originalQty: 10, originalPrice: 102
        }];
    const { points } = await (0, performance_1.calculatePortfolioPerformance)(holdings, txns, 'USD', mockRates, undefined, undefined, mockFetchHistory);
...
async function testCurrencyConversion() {
    ...
    const { points } = await (0, performance_1.calculatePortfolioPerformance)(holdings, txns, 'ILS', mockRates, undefined, undefined, mockFetchHistory);
    const p1 = points.find(p => p.date.toISOString().startsWith('2024-01-02'));
    if (p1) {
...
async function testPartialSell() {
    ...
    const txns = [
        { date: '2024-01-02', portfolioId: 'p1', ticker: 'AAPL', exchange: types_1.Exchange.NASDAQ, type: 'BUY', qty: 10, price: 102 },
        { date: '2024-01-04', portfolioId: 'p1', ticker: 'AAPL', exchange: types_1.Exchange.NASDAQ, type: 'SELL', qty: 5, price: 110 }
    ];
    const { points } = await (0, performance_1.calculatePortfolioPerformance)(holdings, txns, 'USD', mockRates, undefined, undefined, mockFetchHistory);
    const pBefore = points.find(p => p.date.toISOString().startsWith('2024-01-03'));
    if (pBefore)
        assertClose(pBefore.gainsValue, 30, 0.01, 'Gains before sell');
...
async function testDividends() {
    ...
    const txns = [
        { date: '2024-01-02', portfolioId: 'p1', ticker: 'AAPL', exchange: types_1.Exchange.NASDAQ, type: 'BUY', qty: 10, price: 100 },
        { date: '2024-01-03', portfolioId: 'p1', ticker: 'AAPL', exchange: types_1.Exchange.NASDAQ, type: 'DIVIDEND', qty: 0, price: 5, originalPrice: 5 }
    ];
    const { points } = await (0, performance_1.calculatePortfolioPerformance)(holdings, txns, 'USD', mockRates, undefined, undefined, mockFetchHistory);
    const p = points.find(p => p.date.toISOString().startsWith('2024-01-03'));
    if (p)
        assertClose(p.gainsValue, 55, 0.01, 'Gains with dividend');
}
async function testTWR() {
    ...
    const txns = [
        { date: '2024-01-02', portfolioId: 'p1', ticker: 'AAPL', exchange: types_1.Exchange.NASDAQ, type: 'BUY', qty: 10, price: 102 },
        { date: '2024-01-03', portfolioId: 'p1', ticker: 'AAPL', exchange: types_1.Exchange.NASDAQ, type: 'BUY', qty: 10, price: 105 }
    ];
    const { points } = await (0, performance_1.calculatePortfolioPerformance)(holdings, txns, 'USD', mockRates, undefined, undefined, mockFetchHistory);
    const p2 = points.find(p => p.date.toISOString().startsWith('2024-01-03'));
    if (p2) {
...
async function testMissingPriceData() {
    ...
    const holdings = [
        {
            portfolioId: 'p1', ticker: 'GAP', exchange: types_1.Exchange.NASDAQ,
            stockCurrency: types_1.Currency.USD, totalQty: 10
        },
        {
            portfolioId: 'p1', ticker: 'DENSE', exchange: types_1.Exchange.NASDAQ,
            stockCurrency: types_1.Currency.USD, totalQty: 1
        }
    ];
    // Mock history
    const customHistory = {
    ...
    const fetchGap = async (t) => ({ historical: customHistory[`NASDAQ:${t}`], fromCache: true });
    const txns = [
        { date: '2024-01-01', portfolioId: 'p1', ticker: 'GAP', exchange: types_1.Exchange.NASDAQ, type: 'BUY', qty: 10, price: 100 },
        { date: '2024-01-01', portfolioId: 'p1', ticker: 'DENSE', exchange: types_1.Exchange.NASDAQ, type: 'BUY', qty: 1, price: 10 }
    ];
    const { points } = await (0, performance_1.calculatePortfolioPerformance)(holdings, txns, 'USD', mockRates, undefined, undefined, fetchGap);
...
async function testIntradayWash() {
    ...
    const holdings = [{
            portfolioId: 'p1', ticker: 'DAY', exchange: types_1.Exchange.NASDAQ,
            stockCurrency: types_1.Currency.USD, totalQty: 0
        }];
    // Mock history
    const customHistory = {
        'NASDAQ:DAY': [
    ...
    const fetchCustom = async (t) => ({ historical: customHistory[`NASDAQ:${t}`], fromCache: true });
    const txns = [
        { date: '2024-01-02', portfolioId: 'p1', ticker: 'DAY', exchange: types_1.Exchange.NASDAQ, type: 'BUY', qty: 10, price: 100 }, // Cost 1000
        { date: '2024-01-02', portfolioId: 'p1', ticker: 'DAY', exchange: types_1.Exchange.NASDAQ, type: 'SELL', qty: 10, price: 110 } // Proceeds 1100
    ];
    const { points } = await (0, performance_1.calculatePortfolioPerformance)(holdings, txns, 'USD', mockRates, undefined, undefined, fetchCustom);
...
async function testZeroStartTWR() {
    console.log('\n--- Test: Zero Start TWR ---');
    const holdings = [{
            portfolioId: 'p1', ticker: 'ZERO', exchange: types_1.Exchange.NASDAQ,
            stockCurrency: types_1.Currency.USD, totalQty: 10
        }];
    const customHistory = {
        'NASDAQ:ZERO': [
    ...
    const txns = [
        // Day 2: Deposit/Buy
        { date: '2024-01-02', portfolioId: 'p1', ticker: 'ZERO', exchange: types_1.Exchange.NASDAQ, type: 'BUY', qty: 10, price: 100 }
    ];
    const { points } = await (0, performance_1.calculatePortfolioPerformance)(holdings, txns, 'USD', mockRates, undefined, undefined, fetchCustom);
