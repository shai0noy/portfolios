...
import type { DividendRecord } from './data/model';
import { convertCurrency } from './currency';
import type { ExchangeRates } from './types';

export interface DisplayDividendRecord extends DividendRecord {
...
export function aggregateDividends(
    dividendHistory: (DividendRecord & { portfolioCurrency?: string })[],
    displayCurrency: string,
    exchangeRates: ExchangeRates | null
): DisplayDividendRecord[] {
    if (!exchangeRates) return [];

    const grouped: Record<string, DisplayDividendRecord> = {};
    ...
    dividendHistory.forEach(d => {
        ...
        const grossDisplay = convertCurrency(d.grossAmount.amount, d.grossAmount.currency, displayCurrency, exchangeRates);
        const netDisplay = convertCurrency(d.netAmountPC, pCurrency, displayCurrency, exchangeRates);
        const taxDisplay = convertCurrency(d.taxAmountPC, pCurrency, displayCurrency, exchangeRates);
        const feeDisplay = convertCurrency(d.feeAmountPC, pCurrency, displayCurrency, exchangeRates);
        const cashedDisplay = convertCurrency(d.cashedAmount, pCurrency, displayCurrency, exchangeRates);
        const reinvestedDisplay = convertCurrency(d.reinvestedAmount, pCurrency, displayCurrency, exchangeRates);
        ...
        if (hasSplitData) {
            ...
            cashedTaxDisplay = convertCurrency(d.taxCashedPC!, pCurrency, displayCurrency, exchangeRates);
            reinvestedTaxDisplay = convertCurrency(d.taxReinvestedPC!, pCurrency, displayCurrency, exchangeRates);
            
            cashedFeeDisplay = convertCurrency(d.feeCashedPC || 0, pCurrency, displayCurrency, exchangeRates);
            reinvestedFeeDisplay = convertCurrency(d.feeReinvestedPC || 0, pCurrency, displayCurrency, exchangeRates);
